...
| Extension API | Tools, policy, context, verification, and telemetry compose cleanly through public hooks. Replacing low-level request scheduling is beyond those hooks. | Labs 2, 6–10 | **A/C** | Use extensions for local behavior; wrap a component only when changing the internal provider/loop contract. |
...
| Model routing | A controller can choose a model per worker. Transparent per-call routing inside one running Pi agent would require a provider/router component. | Fixed-model workers in Labs 15–20; Pi's model is process/session state. | **B/C** | Route between workers externally; wrap the provider only for within-worker routing. |
...
| Observability | Extension hooks expose rich worker events, but cross-worker traces need shared run/task correlation. | Labs 9, 16, and 20 | **A/B** | Emit inside Pi; aggregate outside Pi. |
